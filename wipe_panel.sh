...
function banner() {
  clear
  echo -e "${YEL}╔═══════════════════════════════════════╗${RST}"
  echo -e "${RED}║     GOLDEN HOSTING TOOLKIT v2.1       ║${RST}"
...
function recreate_user() {
    ...
    echo -e "${BLU}[+] Ensuring user '$USERNAME'@$HOST exists cleanly...${RST}"
    if $MYSQL_CMD -e "SELECT 1 FROM mysql.user WHERE User='$USERNAME' AND Host='$HOST';" | grep -q 1; then
        echo -e "${YEL}[*] User '$USERNAME'@$HOST already exists. Dropping...${RST}"
        $MYSQL_CMD -e "DROP USER '$USERNAME'@'$HOST';" 2>/dev/null
    fi
    $MYSQL_CMD -e "CREATE USER '$USERNAME'@'$HOST' IDENTIFIED BY '$PASSWORD';"
    $MYSQL_CMD -e "FLUSH PRIVILEGES;"
    echo -e "${GRN}[✓] User recreated: '$USERNAME'@$HOST${RST}"
}

# === FIX INVALID DEFINERS ===
function fix_definers() {
    echo -e "${MAG}[+] Checking for invalid DEFINER entries...${RST}"
    DEFINERS=$($MYSQL_CMD -N -e "SELECT CONCAT(ROUTINE_TYPE, ' ', ROUTINE_SCHEMA, '.', ROUTINE_NAME) FROM information_schema.ROUTINES WHERE DEFINER NOT LIKE '%@localhost';" 2>/dev/null)
    ...
    while read -r ENTRY; do
        ...
        FULL=$(echo "$ENTRY" | awk '{print $2}')
        CMD="ALTER $TYPE $FULL COMMENT 'fixed invalid definer';"
        OUTPUT=$($MYSQL_CMD -e "$CMD" 2>&1)
        if [[ "$OUTPUT" == *"ERROR"* ]]; then
            echo -e "${RED}[!] MySQL error: $CMD\n$OUTPUT${RST}"
        else
            echo -e "${GRN}[✓] Fixed: $FULL${RST}"
        fi
    done <<< "$DEFINERS"
}
...
LEFT_USERS=$($MYSQL_CMD -N -e "SELECT CONCAT(User,'@',Host) FROM mysql.user WHERE User NOT IN ('mysql.sys','root','mysql.session','debian-sys-maint');" 2>/dev/null)
...
recreate_user "admin" "localhost" "admin"
recreate_user "admin" "127.0.0.1" "admin"
recreate_user "admin" "private.anonymoushosting.com.ng" "admin"
